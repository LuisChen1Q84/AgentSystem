...
run_agent_repair_compare() {
  ...
  if [ -n "${base_snapshot_id}" ]; then
    cmd+=(--base-snapshot-id "${base_snapshot_id}")
  fi
  if [ -n "${out_dir}" ]; then
    cmd+=(--out-dir "${out_dir}")
  fi
  ...
  "${cmd[@]}" || return "${E_SKILL}"
  automation_log "INFO" "agent-repair-compare" "done"
}
...
Usage:
  ...
  scripts/agentsys.sh agent-studio [repl|run|context-profile|context-scaffold|question-set|question-pending|question-answer|run-resume|session-list|session-view|inbox|action-plan|workbench|observe|recommend|diagnostics|research-report|research-deck|research-lookup|market-report|market-committee|governance|failure-review|repair-apply|repair-approve|repair-list|repair-presets|repair-compare|repair-rollback|run-inspect|slo|policy|pending|feedback-add|feedback-stats|services|call] [args...]
