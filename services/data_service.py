...
class DataService:
    ...
    def query(self, params: Dict[str, Any]) -> ServiceEnvelope:
        ...
        if not spec:
            return error_response("data.query", "missing spec or preset", code="missing_query_spec")
        ...
        if from_date and not _is_valid_date(from_date):
            return error_response("data.query", "--from-date 格式必须为 YYYY-MM-DD", code="invalid_from_date")
        if to_date and not _is_valid_date(to_date):
            return error_response("data.query", "--to-date 格式必须为 YYYY-MM-DD", code="invalid_to_date")
        if from_date and to_date and from_date > to_date:
            return error_response("data.query", "--from-date 不能晚于 --to-date", code="invalid_date_range")
        ...
        return ok_response("data.query", payload={"filters": filters, "items": items}, meta={"db": str(db)})
