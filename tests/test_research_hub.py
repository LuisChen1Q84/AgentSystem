...
class ResearchHubTest(unittest.TestCase):
    def test_systematic_search_generates_review_objects(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(Path(out.get("review_appendix_xlsx_path", "")).exists())
            self.assertTrue(Path(out.get("prisma_svg_path", "")).exists())
    ...
    def test_market_sizing_generates_structured_outputs(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(out.get("candidate_selection", {}).get("selected_id"))
