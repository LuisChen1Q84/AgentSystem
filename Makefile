...
agent-studio:
	@$(ROOT)/scripts/agentsys.sh agent-studio $(or $(cmd),repl) $(if $(text),--text "$(text)",) $(if $(profile),--profile "$(profile)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(status),--status "$(status)",) $(if $(task_kind),--task-kind "$(task_kind)",) $(if $(rating),--rating "$(rating)",) $(if $(run_id),--run-id "$(run_id)",) $(if $(question_set_id),--question-set-id "$(question_set_id)",) $(if $(resume_token),--resume-token "$(resume_token)",) $(if $(answers_json),--answers-json '$(answers_json)',) $(if $(note),--note "$(note)",) $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(context_dir),--context-dir "$(context_dir)",) $(if $(project_name),--project-name "$(project_name)",) $(if $(service),--service "$(service)",) $(if $(params),--params-json '$(params)',) $(if $(dry),--dry-run,) $(if $(apply),--apply,) $(if $(resume),--resume,) $(if $(force),--force,) $(if $(memory_file),--memory-file "$(memory_file)",) $(if $(profile_overrides_file),--profile-overrides-file "$(profile_overrides_file)",) $(if $(strategy_overrides_file),--strategy-overrides-file "$(strategy_overrides_file)",) $(if $(backup_dir),--backup-dir "$(backup_dir)",) $(if $(snapshot_id),--snapshot-id "$(snapshot_id)",) $(if $(base_snapshot_id),--base-snapshot-id "$(base_snapshot_id)",) $(if $(plan_file),--plan-file "$(plan_file)",) $(if $(min_priority_score),--min-priority-score "$(min_priority_score)",) $(if $(max_actions),--max-actions "$(max_actions)",) $(if $(mode),--mode "$(mode)",) $(if $(presets_file),--presets-file "$(presets_file)",) $(if $(top_n),--top-n "$(top_n)",) $(if $(allow_update),--allow-update,) $(if $(include_review_only),--include-review-only,) $(if $(selector_preset),--selector-preset "$(selector_preset)",) $(if $(selector_presets_file),--selector-presets-file "$(selector_presets_file)",) $(if $(min_effectiveness_score),--min-effectiveness-score "$(min_effectiveness_score)",) $(if $(only_if_effective),--only-if-effective,) $(if $(avoid_rolled_back),--avoid-rolled-back,) $(if $(scopes),--scopes "$(scopes)",) $(if $(strategies),--strategies "$(strategies)",) $(if $(task_kinds),--task-kinds "$(task_kinds)",) $(if $(exclude_scopes),--exclude-scopes "$(exclude_scopes)",) $(if $(exclude_strategies),--exclude-strategies "$(exclude_strategies)",) $(if $(exclude_task_kinds),--exclude-task-kinds "$(exclude_task_kinds)",) $(if $(approve_code),--approve-code "$(approve_code)",) $(if $(only),--only "$(only)",)

agent-context-profile:
	@if [ -z "$(context_dir)" ]; then echo "Usage: make agent-context-profile context_dir='/path/to/context'"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio context-profile --context-dir "$(context_dir)"

agent-context-scaffold:
	@if [ -z "$(context_dir)" ]; then echo "Usage: make agent-context-scaffold context_dir='/path/to/context' [project_name='Project'] [force=1]"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio context-scaffold --context-dir "$(context_dir)" $(if $(project_name),--project-name "$(project_name)",) $(if $(force),--force,)

agent-question-set:
	@if [ -z "$(text)" ]; then echo "Usage: make agent-question-set text='task' [context_dir='/path/to/context'] [task_kind='presentation'] [params='{\"k\":\"v\"}']"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio question-set --text "$(text)" $(if $(context_dir),--context-dir "$(context_dir)",) $(if $(task_kind),--task-kind "$(task_kind)",) $(if $(params),--params-json '$(params)',)

agent-question-pending:
	@$(ROOT)/scripts/agentsys.sh agent-studio question-pending $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(limit),--limit "$(limit)",) $(if $(status),--status "$(status)",)

agent-question-answer:
	@if [ -z "$(question_set_id)" ] || [ -z "$(answers_json)" ]; then echo "Usage: make agent-question-answer question_set_id='qs_xxx' answers_json='{\"key\":\"value\"}' [data_dir='...'] [resume=1]"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio question-answer --question-set-id "$(question_set_id)" --answers-json '$(answers_json)' $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(note),--note "$(note)",) $(if $(resume),--resume,)
...
agent-research-report:
	@if [ -z "$(text)" ]; then echo "Usage: make agent-research-report text='<query>' [params='{\"playbook\":\"market_sizing\"}']"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio research-report --text "$(text)" $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(params),--params-json '$(params)',)

agent-research-deck:
	@if [ -z "$(text)" ]; then echo "Usage: make agent-research-deck text='<query>' [params='{\"playbook\":\"ceo_text_deck\"}']"; exit 2; fi
	@$(ROOT)/scripts/agentsys.sh agent-studio research-deck --text "$(text)" $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(params),--params-json '$(params)',)
...
agent-repair-presets:
	@$(ROOT)/scripts/agentsys.sh agent-repair-presets $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(mode),--mode "$(mode)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(presets_file),--presets-file "$(presets_file)",) $(if $(effectiveness_file),--effectiveness-file "$(effectiveness_file)",) $(if $(lifecycle_file),--lifecycle-file "$(lifecycle_file)",) $(if $(top_n),--top-n $(top_n),) $(if $(allow_update),--allow-update,) $(if $(include_review_only),--include-review-only,) $(if $(apply_lifecycle),--apply-lifecycle,)
