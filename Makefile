...
help:
	...
	@echo "  make skill-route text='...' | skill-execute text='...' [params='{\"k\":\"v\"}'] | autonomous text='...' [params='{\"k\":\"v\"}'] | agent text='...' [params='{\"profile\":\"strict|adaptive|auto\"}'] | agent-studio [cmd='repl|run|observe|recommend|diagnostics|failure-review|repair-apply|repair-approve|repair-list|repair-presets|repair-compare|repair-rollback|run-inspect|slo|policy|pending|feedback-add|feedback-stats|services|call'] [service='mcp.run|ppt.generate|image.generate|market.report|data.query|agent.diagnostics|agent.failures.review|agent.repairs.apply|agent.repairs.approve|agent.repairs.list|agent.repairs.presets|agent.repairs.compare|agent.repairs.rollback|agent.run.inspect|agent.policy.tune'] [params='{\"k\":\"v\"}'] [selector_preset='presentation_recovery|auto'] [selector_presets_file='.../agent_repair_selector_presets.json'] [min_effectiveness_score=5] [only_if_effective=1] [avoid_rolled_back=1] | agent-observe [days=14] | agent-recommend [days=30] [apply=1] | agent-failure-review [days=14] [limit=10] | agent-repair-apply [days=14] [limit=10] [min_priority_score=50] [max_actions=3] [selector_preset='presentation_recovery|auto'] [selector_presets_file='.../agent_repair_selector_presets.json'] [min_effectiveness_score=5] [only_if_effective=1] [avoid_rolled_back=1] [scopes='strategy,task_kind'] [strategies='mckinsey-ppt'] [task_kinds='presentation'] [exclude_scopes='feedback'] [exclude_strategies='mcp-generalist'] [exclude_task_kinds='report'] [apply=1] [snapshot_id='...'] [approve_code='...'] | agent-repair-approve [days=14] [limit=10] [min_priority_score=50] [max_actions=3] [selector_preset='presentation_recovery|auto'] [selector_presets_file='.../agent_repair_selector_presets.json'] [min_effectiveness_score=5] [only_if_effective=1] [avoid_rolled_back=1] [scopes='strategy,task_kind'] [strategies='mckinsey-ppt'] [task_kinds='presentation'] [exclude_scopes='feedback'] [exclude_strategies='mcp-generalist'] [exclude_task_kinds='report'] [snapshot_id='...'] [approve_code='...'] | agent-repair-list [limit=20] | agent-repair-presets [mode='list|recommend|save'] [top_n=3] [presets_file='.../agent_repair_selector_presets.json'] [allow_update=1] [include_review_only=1] | agent-repair-compare [snapshot_id='...'] [base_snapshot_id='...'] | agent-repair-rollback [snapshot_id='...'] [only='both|profile|strategy'] | agent-run-inspect run_id='...' | agent-policy [days=14] | agent-pack cmd='list|enable|disable' [name='finance'] | agent-slo-guard [enforce=1] | agent-golden [strict=1] | agent-fault [strict=1] | agent-feedback cmd='add|stats|pending' [run_id='...'] [rating='1|-1'] | agent-learn [apply=1] | capability-catalog | skill-contract-lint [strict=1] | autonomy-observe [days=14] | autonomy-eval | image-hub text='...' [params='{\"k\":\"v\"}'] | image-hub-observe [days=7]"
...
agent-studio:
	@$(ROOT)/scripts/agentsys.sh agent-studio $(or $(cmd),repl) $(if $(text),--text "$(text)",) $(if $(profile),--profile "$(profile)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(task_kind),--task-kind "$(task_kind)",) $(if $(rating),--rating "$(rating)",) $(if $(run_id),--run-id "$(run_id)",) $(if $(note),--note "$(note)",) $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(service),--service "$(service)",) $(if $(params),--params-json '$(params)',) $(if $(dry),--dry-run,) $(if $(apply),--apply,) $(if $(force),--force,) $(if $(memory_file),--memory-file "$(memory_file)",) $(if $(profile_overrides_file),--profile-overrides-file "$(profile_overrides_file)",) $(if $(strategy_overrides_file),--strategy-overrides-file "$(strategy_overrides_file)",) $(if $(backup_dir),--backup-dir "$(backup_dir)",) $(if $(snapshot_id),--snapshot-id "$(snapshot_id)",) $(if $(base_snapshot_id),--base-snapshot-id "$(base_snapshot_id)",) $(if $(plan_file),--plan-file "$(plan_file)",) $(if $(min_priority_score),--min-priority-score "$(min_priority_score)",) $(if $(max_actions),--max-actions "$(max_actions)",) $(if $(mode),--mode "$(mode)",) $(if $(presets_file),--presets-file "$(presets_file)",) $(if $(top_n),--top-n "$(top_n)",) $(if $(allow_update),--allow-update,) $(if $(include_review_only),--include-review-only,) $(if $(selector_preset),--selector-preset "$(selector_preset)",) $(if $(selector_presets_file),--selector-presets-file "$(selector_presets_file)",) $(if $(min_effectiveness_score),--min-effectiveness-score "$(min_effectiveness_score)",) $(if $(only_if_effective),--only-if-effective,) $(if $(avoid_rolled_back),--avoid-rolled-back,) $(if $(scopes),--scopes "$(scopes)",) $(if $(strategies),--strategies "$(strategies)",) $(if $(task_kinds),--task-kinds "$(task_kinds)",) $(if $(exclude_scopes),--exclude-scopes "$(exclude_scopes)",) $(if $(exclude_strategies),--exclude-strategies "$(exclude_strategies)",) $(if $(exclude_task_kinds),--exclude-task-kinds "$(exclude_task_kinds)",) $(if $(approve_code),--approve-code "$(approve_code)",) $(if $(only),--only "$(only)",)
...
agent-repair-apply:
	@$(ROOT)/scripts/agentsys.sh agent-repair-apply $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(apply),--apply,) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(profile_overrides_file),--profile-overrides-file "$(profile_overrides_file)",) $(if $(strategy_overrides_file),--strategy-overrides-file "$(strategy_overrides_file)",) $(if $(backup_dir),--backup-dir "$(backup_dir)",) $(if $(snapshot_id),--snapshot-id "$(snapshot_id)",) $(if $(plan_file),--plan-file "$(plan_file)",) $(if $(min_priority_score),--min-priority-score "$(min_priority_score)",) $(if $(max_actions),--max-actions "$(max_actions)",) $(if $(selector_preset),--selector-preset "$(selector_preset)",) $(if $(selector_presets_file),--selector-presets-file "$(selector_presets_file)",) $(if $(min_effectiveness_score),--min-effectiveness-score "$(min_effectiveness_score)",) $(if $(only_if_effective),--only-if-effective,) $(if $(avoid_rolled_back),--avoid-rolled-back,) $(if $(scopes),--scopes "$(scopes)",) $(if $(strategies),--strategies "$(strategies)",) $(if $(task_kinds),--task-kinds "$(task_kinds)",) $(if $(exclude_scopes),--exclude-scopes "$(exclude_scopes)",) $(if $(exclude_strategies),--exclude-strategies "$(exclude_strategies)",) $(if $(exclude_task_kinds),--exclude-task-kinds "$(exclude_task_kinds)",) $(if $(approve_code),--approve-code "$(approve_code)",) $(if $(force),--force,)
...
agent-repair-presets:
	@$(ROOT)/scripts/agentsys.sh agent-repair-presets $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(mode),--mode "$(mode)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(presets_file),--presets-file "$(presets_file)",) $(if $(top_n),--top-n $(top_n),) $(if $(allow_update),--allow-update,) $(if $(include_review_only),--include-review-only,)
