...
agent-repair-apply:
	@$(ROOT)/scripts/agentsys.sh agent-repair-apply $(if $(data_dir),--data-dir "$(data_dir)",) $(if $(days),--days $(days),) $(if $(limit),--limit $(limit),) $(if $(apply),--apply,) $(if $(out_dir),--out-dir "$(out_dir)",) $(if $(profile_overrides_file),--profile-overrides-file "$(profile_overrides_file)",) $(if $(strategy_overrides_file),--strategy-overrides-file "$(strategy_overrides_file)",) $(if $(backup_dir),--backup-dir "$(backup_dir)",) $(if $(snapshot_id),--snapshot-id "$(snapshot_id)",) $(if $(plan_file),--plan-file "$(plan_file)",) $(if $(approve_code),--approve-code "$(approve_code)",) $(if $(force),--force,)
