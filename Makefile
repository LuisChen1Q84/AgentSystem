...
help:
	...
	@echo "  make skill-route text='...' | skill-execute text='...' [params='{\"k\":\"v\"}'] | autonomous text='...' [params='{\"k\":\"v\"}'] | agent text='...' [params='{\"profile\":\"strict|adaptive|auto\"}'] | agent-studio [cmd='repl|run|observe|recommend|state-sync|state-stats|diagnostics|research-report|governance|failure-review|repair-observe|repair-apply|repair-approve|repair-list|repair-presets|repair-compare|repair-rollback|run-inspect|object-view|run-replay|slo|policy|policy-apply|preferences|pending|feedback-add|feedback-stats|services|call'] [service='mcp.run|ppt.generate|image.generate|market.report|research.report|data.query|agent.diagnostics|agent.governance.console|agent.failures.review|agent.repairs.observe|agent.repairs.apply|agent.repairs.approve|agent.repairs.list|agent.repairs.presets|agent.repairs.compare|agent.repairs.rollback|agent.run.inspect|agent.object.view|agent.run.replay|agent.policy.tune|agent.policy.apply|agent.state.sync|agent.state.stats|agent.preferences.learn'] [params='{\"k\":\"v\"}']"
